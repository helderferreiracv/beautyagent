...
import { ArrowLeft, ArrowRight, Clock, Sparkles, Ban, Phone, Check, Plus, Zap, Receipt } from 'lucide-react';
import { Button } from '../../components/Button';
import { BookingProgressBar } from '../../components/BookingProgressBar';
import { Service } from '../../types';

export const ClientServiceSelection: React.FC = () => {
  ...
  const handleContinue = () => {
    ...
    const totalPrice = selectedServices.reduce((acc, s) => acc + parseFloat(s.price.replace('€','')), 0);
  ...
  const stats = {
    ...
    price: selectedServices.reduce((acc, s) => acc + parsePrice(s.price), 0),
    duration: selectedServices.reduce((acc, s) => acc + parseInt(s.duration), 0)
  };
  ...
  if (isBlocked) {
    return (
      <div className="max-w-md mx-auto py-20 px-6 animate-fade-in text-center">
        ...
        <Button onClick={() => window.location.href = 'tel:912345678'} className="w-full h-14 text-xs font-black uppercase tracking-widest">
  ...
  const SummaryContent = () => (
    <>
      <div className="flex justify-between items-end mb-6">
         <div>
            <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest block mb-1">Total Estimado</span>
            <div className="flex items-baseline gap-2">
               <span className="text-3xl font-black text-white italic tracking-tighter">{stats.price}€</span>
      ...
      <Button 
        ...
        className={`py-5 shadow-2xl text-xs transition-all ${stats.count === 0 ? 'opacity-50 grayscale cursor-not-allowed' : 'active:scale-95'}`}
      >
  ...
  return (
    <div className="max-w-5xl mx-auto py-8 px-4 md:px-8 animate-fade-in pb-40 md:pb-12">
      {/* Header Navigation */}
      <button onClick={() => navigate('/')} className="mb-8 text-muted hover:text-white flex items-center gap-2 text-[10px] font-black uppercase tracking-widest transition-all group">
        <ArrowLeft size={14} className="group-hover:-translate-x-1 transition-transform" /> Voltar
      </button>

      <div className="max-w-xl mb-12">
         <h1 className="text-4xl md:text-5xl font-black text-white italic uppercase tracking-tighter leading-none mb-4">Menu</h1>
         <p className="text-muted text-sm font-medium">Seleciona os tratamentos que desejas realizar hoje.</p>
      </div>

      {/* Grid Layout: Desktop (2 Columns) vs Mobile (1 Column) */}
                ...
                >
                  ...
                  <div className="flex-grow min-w-0">
                    <div className="flex justify-between items-start mb-1">
                       <span className={`text-sm md:text-base font-black uppercase italic tracking-tight truncate pr-2 ${isSelected ? 'text-white' : 'text-zinc-300 group-hover:text-white'}`}>
                         {service.name}
                       </span>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                       <div className="flex items-center gap-1.5 bg-background/50 px-2 py-1 rounded-lg border border-white/5">
                          ...
                          <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">{service.duration}m</span>
                       </div>
                       <span className={`text-[11px] font-black tracking-widest ${isSelected ? 'text-primary' : 'text-zinc-500'}`}>
                          {service.price}€
                       </span>
                    </div>
                  </div>
                  ...
                  {isSelected && (
                     <div className="absolute right-0 top-0 p-3">
                        <div className="w-2 h-2 bg-primary rounded-full animate-pulse shadow-[0_0_10px_#ebff57]"></div>
                     </div>
                  )}
                </button>
              );
        ...
        <div className="hidden md:block md:col-span-5 lg:col-span-4">
           <div className="sticky top-24">
              <div className="bg-surface border border-white/10 rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden">
                 <div className="absolute top-0 right-0 p-6 opacity-5 pointer-events-none"><Receipt size={64} /></div>
                 ...
                 {selectedServices.length > 0 ? (
                    <div className="space-y-4 mb-8 max-h-[30vh] overflow-y-auto pr-2 scrollbar-hide">
                       {selectedServices.map(s => (
                          <div key={s.id} className="flex justify-between items-center text-sm group">
                             <span className="text-zinc-300 font-medium truncate pr-4 group-hover:text-white transition-colors">{s.name}</span>
                             <span className="text-zinc-500 font-bold whitespace-nowrap">{s.price}€</span>
                          </div>
                       ))}
                    ...
                    <div className="py-12 text-center text-zinc-600">
                       <p className="text-xs font-medium italic">Seleciona um serviço à esquerda para começar.</p>
                    </div>
                 )}
              </div>
           </div>
