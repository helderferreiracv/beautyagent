    ...
    <div className="relative overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-50 bg-[#050505]/90 backdrop-blur-xl border-b border-white/5 h-20">
         <div className="max-w-7xl mx-auto px-6 h-full flex items-center justify-between">
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => router.refresh()}>
               ...
               <span className="text-xl font-black text-white italic uppercase tracking-tighter">BeautyAgent</span>
            </div>
            <div className="flex items-center gap-4 md:gap-10">
               <button onClick={() => router.push('/owner/dashboard')} className="text-xs font-black text-zinc-300 hover:text-white uppercase tracking-widest transition-colors">Login</button>
               <Button onClick={() => router.push('/trial/register')} className="shadow-xl px-8 h-11 text-xs">Trial Grátis</Button>
            </div>
         </div>
      </nav>
      ...
      <section className="relative pt-48 pb-24 px-6 z-10 text-center max-w-6xl mx-auto">
         ...
         <p className="text-zinc-300 text-lg md:text-2xl font-medium max-w-3xl mx-auto mb-16 leading-relaxed">
            O BeautyAgent é o teu agente AI para marcações, fidelização e otimização da agenda. <br className="hidden md:block" />
            <span className="text-white font-bold underline decoration-[#ebff57]/40 underline-offset-8">Simples, moderno e eficiente.</span>
         </p>

         <div className="flex flex-col items-center gap-8">
            <Button onClick={() => router.push('/trial/register')} className="w-full sm:w-auto px-16 py-8 text-base shadow-[0_0_50px_rgba(235,255,87,0.25)] hover:scale-105">
      ...
      <footer className="border-t border-white/5 bg-[#050505] pt-32 pb-16 px-6 relative z-10">
         <div className="max-w-7xl mx-auto text-center">
            ...
            <button onClick={() => setIsAdminModalOpen(true)} className="mt-8 text-xs text-zinc-800 hover:text-[#ebff57] font-black uppercase tracking-widest">Admin</button>
         </div>
      </footer>
      ...
      {isAdminModalOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/98 backdrop-blur-xl" onClick={() => setIsAdminModalOpen(false)}></div>
          <div className="bg-[#343436] w-full max-w-[380px] rounded-[3rem] p-10 relative z-10 border border-white/10 shadow-2xl">
             ...
             <form onSubmit={handleAdminAccess} className="space-y-8">
                <Input type="password" placeholder="CHAVE MESTRE" value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} className="text-center tracking-[0.6em] font-mono text-xl h-16" autoFocus />
                <Button fullWidth type="submit" className="py-5">Autenticar Sistema</Button>
             </form>
          </div>
        </div>
      )}
